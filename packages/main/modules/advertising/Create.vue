  ...
  >
    ...
    >
      <el-form-item label="计划标题" prop="title">
        <el-input v-model="form.title" placeholder="如：京东商城6.18活动推广" />
      </el-form-item>
...
<script lang="ts" setup>
  ...
  const initForm = function (): AdvertisingPlan.CreateData {
    return {
      title: '',
      timeRange: [],
      channelRange: [],
  ...
  const handleConfirm = (formEl: FormInstance | undefined) => {
    formEl?.validate((valid) => {
      if (valid) {
        requestV2.adCreate({ data: form }).then(() => {
