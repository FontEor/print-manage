...
<script lang="ts" setup>
  ...
  const getVersionList = async () => {
    ...
    const params = { from: state.page.from, size: state.page.size, tempType: 2 };
  ...
  const drawTemplate = (row: Version.Instance) => {
    window.open(
      `/editor/?tempType=2&orgId=${state.id}&temId=${row.tempId}&sourceId=${
        row.sourceId || ''
      }&verId=${row.id || ''}&cid=${row.customTempId}&templateCode=${row.code}`,
      '_blank',
    );
  };
